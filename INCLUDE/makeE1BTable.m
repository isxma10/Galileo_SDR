function e1bCodesTable = makeE1BTable(settings)
...
samplesPerCode = round(settings.samplingFreq / ...
                           (settings.codeFreqBasisE1B / settings.codeLengthE1B));

%--- Prepare the output matrix to speed up function -----------------------
e1bCodesTable = zeros(32, 1*samplesPerCode);
...
%=== For all satellite PRN-s ...
for PRN = 1:32
    %--- Generate E1B code for given PRN -----------------------------------
    
    e1bCode = generateE1Bcode(PRN,1);
    %e1BCode = generateE1bcodeMultipath(PRN, 1);
    %=== Digitizing =======================================================
    ...
    % millisecond).
    codeValueIndex = ceil(4*(ts * (1:samplesPerCode)) / tc);
    
    %--- Correct the last index (due to number rounding issues) -----------
    codeValueIndex(end) = 4*4092;
